...
def load_known_urls():
    """Return the set of all document URLs from the previous scraped_materials.json."""
    if not OUTPUT_FILE.exists():
        return set()
    with open(OUTPUT_FILE, encoding="utf-8") as f:
        old = json.load(f)
    urls = set()
    for cdata in old.get("committees", {}).values():
...
def load_recent_new_docs(within_hours=24):
    """Return new-doc entries from the previous new_materials.json still within the time window."""
    if not NEW_MATERIALS_FILE.exists():
        return []
    with open(NEW_MATERIALS_FILE, encoding="utf-8") as f:
        data = json.load(f)
    cutoff = datetime.now() - timedelta(hours=within_hours)
    return [
        d for d in data.get("new_docs", [])
        if datetime.fromisoformat(d["found_at"]) >= cutoff
    ]
...
async def main(months_back):
    ...
    # Load previous state before overwriting
    old_urls       = load_known_urls()
    recent_new     = load_recent_new_docs()
    ...
    # ── Detect new documents (URLs not in previous scrape) ────────────────────
    new_docs = []
    for cid, cdata in results["committees"].items():
        for docs in cdata["meetings"].values():
            for doc in docs:
                if doc["url"] not in old_urls:
                    new_docs.append({
                        "committee_id":   cid,
    ...
    # Merge with recent new docs from previous runs (avoid duplicates)
    this_run_urls = {d["url"] for d in new_docs}
    for d in recent_new:
        if d["url"] not in this_run_urls:
            new_docs.append(d)
            this_run_urls.add(d["url"])
    ...
    print(f"New documents this run: {len([d for d in new_docs if d['found_at'] == now_iso])}")
    print(f"New materials (24h window total): {len(new_docs)}")
